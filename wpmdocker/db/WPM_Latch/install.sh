...
sed -i "s|%APPID%|$LATCHAPPID|g" $INST/token.rb 
sed -i "s|%SECRET%|$LATCHSECRET|g" $INST/token.rb

cp $INST/operations_template.rb $INST/operations.rb
sed -i "s|%APPID%|$LATCHAPPID|g" $INST/operations.rb
sed -i "s|%SECRET%|$LATCHSECRET|g" $INST/operations.rb
...
mysql -u root -p$MYSQL_ROOT_PASSWORD < lib_mysqludf_sys.sql
...
mysql -u root -p$MYSQL_ROOT_PASSWORD wordpress < $INST/proof.sql
if [ $? -eq 0 ]
then
